...
abstract class qtype_clickhotspot_shape {
    ...
    public function get_coords_interpreter_error() {
        if ($this->error) {
            ...
            return get_string('formerror_'.$this->error, 'qtype_clickhotspot', $a);
        } else {
            return false;
        }
    }
    ...
    protected static $classnameprefix = 'qtype_clickhotspot_shape_';
    ...
    public static function human_readable_name($lowercase = false) {
        ...
        return get_string($stringid, 'qtype_clickhotspot');
    }

    public static function human_readable_coords_format() {
        return get_string('shape_'.self::name().'_coords', 'qtype_clickhotspot');
    }
...
class qtype_clickhotspot_shape_polygon extends qtype_clickhotspot_shape {
    ...
    public function is_point_in_shape($xy) {
        $pointatinfinity = new qtype_clickhotspot_point(-1000000, $xy[1]+1);
        $pointtotest = new qtype_clickhotspot_point($xy[0], $xy[1]);
        $testsegment = new qtype_clickhotspot_segment($pointatinfinity, $pointtotest);
        $windingnumber = 0;
        foreach ($this->coords as $index => $coord) {
            if ($index != 0) {
                $a = new qtype_clickhotspot_point($this->coords[$index -1][0],
                                                $this->coords[$index -1][1]);
            } else {
                $a = new qtype_clickhotspot_point($this->coords[count($this->coords)-1][0],
                                                $this->coords[count($this->coords)-1][1]);
            }
            $b = new qtype_clickhotspot_point($this->coords[$index][0],
                                            $this->coords[$index][1]);
            $segment = new qtype_clickhotspot_segment($a, $b);
     ...
     * @param qtype_clickhotspot_segment $v
     * @param int $ua
     * @param int $ub
     */
    public function perturb($p, $q) {
        ...
        if ($ua == 0) {
            ...
            $a = ($pt * $p->a->dist(new qtype_clickhotspot_point($p->b->x, $p->a->y)))/$h;
            $b = ($pt * $p->b->dist(new qtype_clickhotspot_point($p->b->x, $p->a->y)))/$h;
            $p->a->x = $p->a->x + $a;
            $p->a->y = $p->a->y + $b;
        } else if ($ua == 1) {
            ...
            $a = ($pt * $q->a->dist(new qtype_clickhotspot_point($q->b->x, $q->a->y)))/$h;
            $b = ($pt * $q->b->dist(new qtype_clickhotspot_point($q->b->x, $q->a->y)))/$h;
            $q->a->x = $q->a->x + $a;
            $q->a->y = $q->a->y + $b;
        } else if ($ub == 1) {
...
class qtype_clickhotspot_segment {
    ...
    public function __construct(qtype_clickhotspot_point $a, qtype_clickhotspot_point $b) {
        $this->a = $a;
        $this->b = $b;
    }
    /**
     ...
     */
    public function intersects(qtype_clickhotspot_segment $v) {
    ...
    public function intersection_point(qtype_clickhotspot_segment $v) {
