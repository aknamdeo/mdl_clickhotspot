...
define('QTYPE_CLICKHOTSPOT_BGIMAGE_MAXWIDTH', 600);
define('QTYPE_CLICKHOTSPOT_BGIMAGE_MAXHEIGHT', 400);
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class question_hint_clickhotspot extends question_hint_with_parts {
     ...
     * @return question_hint_clickhotspot
     */
    public static function load_from_record($row) {
        return new question_hint_clickhotspot($row->id, $row->hint, $row->hintformat,
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_clickhotspot extends qtype_ddtoimage_base {

    public function save_question_options($formdata) {
        global $DB, $USER;
        $context = $formdata->context;

        $options = $DB->get_record('qtype_clickhotspot', array('questionid' => $formdata->id));
        if (!$options) {
            ...
            $options->id = $DB->insert_record('qtype_clickhotspot', $options);
        }
        ...
        $DB->update_record('qtype_clickhotspot', $options);
        $DB->delete_records('qtype_clickhotspot_drops', array('questionid' => $formdata->id));
        ...
        $olddragids = $DB->get_records_menu('qtype_clickhotspot_drags',
        ...
        foreach (array_keys($formdata->drags) as $dragno) {
            if (!empty($formdata->drags[$dragno]['label'])) {
                ...
                if (isset($olddragids[$dragno +1])) {
                    ...
                    $DB->update_record('qtype_clickhotspot_drags', $drag);
                } else {
                    $drag->id = $DB->insert_record('qtype_clickhotspot_drags', $drag);
                }
        ...
        if (!empty($olddragids)) {
            ...
            $DB->delete_records_select('qtype_clickhotspot_drags', "id $sql", $params);
        }

        self::constrain_image_size_in_draft_area($formdata->bgimage,
                                                    QTYPE_CLICKHOTSPOT_BGIMAGE_MAXWIDTH,
                                                    QTYPE_CLICKHOTSPOT_BGIMAGE_MAXHEIGHT);
        file_save_draft_area_files($formdata->bgimage, $formdata->context->id,
                                    'qtype_clickhotspot', 'bgimage', $formdata->id,
    ...
    protected function make_hint($hint) {
        return question_hint_clickhotspot::load_from_record($hint);
    }
    protected function make_choice($dragdata) {
        return new qtype_clickhotspot_drag_item($dragdata->label, $dragdata->no, $dragdata->infinite, $dragdata->noofdrags);
    }

    protected function make_place($dropdata) {
        return new qtype_clickhotspot_drop_zone($dropdata->no, $dropdata->shape, $dropdata->coords);
    }
    ...
    public function move_files($questionid, $oldcontextid, $newcontextid) {
        ...
        $fs->move_area_files_to_new_context($oldcontextid,
                                    $newcontextid, 'qtype_clickhotspot', 'bgimage', $questionid);
    ...
    public function export_to_xml($question, qformat_xml $format, $extra = null) {
        ...
        $output .= $format->write_combined_feedback($question->options,
                                                    $question->id,
                                                    $question->contextid);
        $files = $fs->get_area_files($contextid, 'qtype_clickhotspot', 'bgimage', $question->id);
        $output .= "    ".$this->write_files($files, 2)."\n";;

        foreach ($question->options->drags as $drag) {
            $files =
                    $fs->get_area_files($contextid, 'qtype_clickhotspot', 'dragimage', $drag->id);
            $output .= "    <drag>\n";
    ...
    public function import_from_xml($data, $question, qformat_xml $format, $extra=null) {
        if (!isset($data['@']['type']) || $data['@']['type'] != 'clickhotspot') {
            return false;
        }

        $question = $format->import_headers($data);
        $question->qtype = 'clickhotspot';
