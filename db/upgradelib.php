 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_clickhotspot_qe2_attempt_updater extends question_qtype_attempt_updater {
    public function right_answer() {
        $drag = reset($this->question->options->drags);
        return '{'.get_string('dropzone', 'qtype_clickhotspot', '1')." -> ".$drag->label.'}';
    }
        ...
        if (!empty($state->answer)) {
            ...
            foreach ($this->question->options->drops as $drop) {
                ...
                if (($answerx >= $x && $answerx <= ($x + $w)) && ($answery >= $y && $answery <= ($y + $h))) {
                    return '{'.get_string('dropzone', 'qtype_clickhotspot', $drop->no)." -> ".$drag->label.'}';
                }
            }
            return '';
        } else {
    ...
    public function question_summary() {
        $drag = reset($this->question->options->drags);
        return parent::question_summary().'[['.get_string('dropzone', 'qtype_clickhotspot', '1')."]] -> {".$drag->label.'}';
    }
