...
class qtype_clickhotspot_question_converter_list_item extends qtype_clickhotspot_question_list_item {
    ...
    public function process($renderer) {
        qtype_clickhotspot_convert_image_target_question($this->record, $this->imagetargetrecord->qimage, $this->answers);
...
admin_externalpage_setup('qtypeclickhotspotfromimagetarget');

// Header.
$renderer = $PAGE->get_renderer('qtype_clickhotspot', 'list');
echo $renderer->header();
echo $renderer->heading(get_string('imagetargetconverter', 'qtype_clickhotspot'), 2);
...
if (!count($questions)) {
    echo html_writer::tag('div', get_string('noquestionsfound', 'qtype_clickhotspot'));
} else {
    $contextids = array();
    ...
    $contextlist = new qtype_clickhotspot_context_list($pagestate, $link, array_unique($contextids));
    $categorylist = new qtype_clickhotspot_category_list($pagestate, $link, $contextids, $contextlist);
    $questionlist = new qtype_clickhotspot_question_converter_list($pagestate, $link, $questions, $categorylist);
    ...
    switch ($pagestate) {
        case 'listall' :
            echo $renderer->render_qtype_clickhotspot_list($top);
            break;
        case 'confirm' :
            echo $renderer->render_qtype_clickhotspot_list($top);
            ...
            echo $renderer->confirm(get_string('confirmimagetargetconversion', 'qtype_clickhotspot'), $cofirmedurl, $cancelurl);
            break;
        case 'processing' :
