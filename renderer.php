 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_clickhotspot_renderer extends qtype_ddtoimage_renderer_base {
    public function formulation_and_controls(question_attempt $qa,
        ...
        $img = html_writer::empty_tag('img',
                                        ...
                                        'alt' => get_string('dropbackground', 'qtype_clickhotspot')));
        ...
        $PAGE->requires->yui_module('moodle-qtype_clickhotspot-dd',
                                        'M.qtype_clickhotspot.init_question',
                                        array($params));
    ...
    protected function hint(question_attempt $qa, question_hint $hint) {
        ...
        if ($hint->statewhichincorrect) {
            ...
            $output .= html_writer::nonempty_tag('div',
                    get_string('followingarewrong', 'qtype_clickhotspot', join(', ', $wrongparts)),
                    array('class' => 'wrongparts'));
        }
...
class qtype_clickhotspot_list_renderer extends plugin_renderer_base {

    public function render_qtype_clickhotspot_list(qtype_clickhotspot_list_item $top) {
        $list = html_writer::tag('ul', html_writer::tag('li', $this->render_qtype_clickhotspot_list_item($top)));
        return $this->output->container($list, 'listofquestions');
    }
    public function render_qtype_clickhotspot_list_item(qtype_clickhotspot_list_item $listitem) {
        return $this->item($listitem).$this->children($listitem);
    }

    public function item(qtype_clickhotspot_list_item $item) {
        global $PAGE;
        $a = new stdClass();
        $a->qcount = $item->get_q_count();
        $a->name = $item->item_name();
        $thisitem = get_string('listitem'.$item->get_string_identifier().$item->get_list_type(), 'qtype_clickhotspot', $a);
        if ($item->get_linked()) {
    ...
    protected function children(qtype_clickhotspot_list_item $item) {
        $children = array();
        foreach ($item->get_children() as $child) {
            $children[] = $this->render_qtype_clickhotspot_list_item($child);
        }
